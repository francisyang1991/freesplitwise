...
describe('Invite code generation', () => {
  ...
  describe('generateUniqueInviteCode', () => {
    it('should generate unique code when no conflict exists', async () => {
      ...
      const code = await generateUniqueInviteCode(mockPrisma as any, 6, 5)
    ...
    it('should throw error after max attempts', async () => {
      ...
      await expect(
        generateUniqueInviteCode(mockPrisma as any, 6, 3)
    ...
    it('should use default parameters', async () => {
      ...
      const code = await generateUniqueInviteCode(mockPrisma as any)
    ...
    it('should handle custom length and max attempts', async () => {
      ...
      const code = await generateUniqueInviteCode(mockPrisma as any, 8, 10)
    ...
    it('should handle database errors gracefully', async () => {
      ...
      await expect(
        generateUniqueInviteCode(mockPrisma as any, 6, 1)
